...
if (-not $PythonCmd) { Red "  [FAIL] Python 3 not found. Install from https://python.org"; exit 1 }
Green "  [OK] $PythonCmd $(& $PythonCmd --version 2>&1)"
...
Green "  [OK] mcp, pyyaml, pydantic ready"
...
if ($claudeDetected) {
    ...
    Green "  [OK] Claude Desktop  ->  $claudeConfig"
    $results += [PSCustomObject]@{ Agent = "Claude Desktop"; Status = "[OK] Registered"; Config = $claudeConfig }
} else {
...
if ($cursorDetected) {
    ...
    Green "  [OK] Cursor           ->  $cursorConfig"
    $results += [PSCustomObject]@{ Agent = "Cursor"; Status = "[OK] Registered"; Config = $cursorConfig }
} else {
...
if ($windsurfDetected) {
    ...
    Green "  [OK] Windsurf         ->  $windsurfConfig"
    $results += [PSCustomObject]@{ Agent = "Windsurf"; Status = "[OK] Registered"; Config = $windsurfConfig }
} else {
...
if ($vsCodeDetected) {
    ...
    Green "  [OK] VS Code (Cline)  ->  $clineConfig"
    $results += [PSCustomObject]@{ Agent = "VS Code / Cline"; Status = "[OK] Registered"; Config = $clineConfig }
} else {
...
if ($zedDetected) {
    ...
    Green "  [OK] Zed              ->  $zedConfig"
    $results += [PSCustomObject]@{ Agent = "Zed"; Status = "[OK] Registered"; Config = $zedConfig }
} else {
...
if ($continueDetected) {
    ...
    Green "  [OK] Continue.dev     ->  $continueConfig"
    $results += [PSCustomObject]@{ Agent = "Continue.dev"; Status = "[OK] Registered"; Config = $continueConfig }
} else {
...
# -- Summary table ------------------------------------------------------------
Write-Host ""
$registered = ($results | Where-Object { $_.Status -like "*Registered*" }).Count
...
if ((Test-Path $claudeExePath) -and $registered -gt 0) {
    ...
    if ($ans -eq "" -or $ans -match "^[Yy]") {
        ...
        Green "  [OK] Claude Desktop restarted."
    }
}
Write-Host ""
